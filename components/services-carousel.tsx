...
export function ServicesCarousel({
  ...
  /** When set, an extra card is appended to the row linking to the full,
   *  unlimited listing for this category. */
  seeAllHref?: string;
}) {
    ...
    <div className="group/carousel relative">
      ...
      >
        {items.map((o) => (
          <article
            key={o.id}
            className="group w-44 shrink-0 snap-start transition-transform duration-200 ease-out hover:-translate-y-0.5 sm:w-52 lg:w-60"
          >
        ...
        {seeAllHref && (
          <Link
            ...
            className="group w-44 shrink-0 snap-start transition-transform duration-200 ease-out hover:-translate-y-0.5 sm:w-52 lg:w-60"
          >
            <span className="flex aspect-square flex-col items-center justify-center gap-1 rounded-2xl bg-[#fff7f1] text-center transition-colors duration-200 ease-out group-hover:bg-orange-50 dark:bg-[#1e1a15] dark:group-hover:bg-white/10">
              {/* eslint-disable-next-line @next/next/no-img-element -- decorative SVG sketch, not an optimizable photo */}
              <img
                src="/Marketing_assets/see-all-sketch.svg"
                alt=""
                aria-hidden="true"
                className="h-24 w-24 transition-transform duration-200 ease-out group-hover:scale-105 sm:h-28 sm:w-28"
              />
      ...
      <button
        ...
        className={`absolute left-1 top-[38%] z-20 hidden h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-black/5 bg-[#fff7f1] text-foreground shadow-md transition-all duration-200 ease-out hover:scale-105 hover:bg-orange-50 md:flex dark:border-white/10 dark:bg-[#1e1a15] dark:hover:bg-white/10 ${
          atStart
      ...
      <button
        ...
        className={`absolute right-1 top-[38%] z-20 hidden h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-black/5 bg-[#fff7f1] text-foreground shadow-md transition-all duration-200 ease-out hover:scale-105 hover:bg-orange-50 md:flex dark:border-white/10 dark:bg-[#1e1a15] dark:hover:bg-white/10 ${
          atEnd
            ? "pointer-events-none opacity-0"
            : "opacity-0 group-hover/carousel:opacity-100 focus:opacity-100"
        }`}
