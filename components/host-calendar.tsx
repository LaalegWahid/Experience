  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-full text-zinc-500 transition-colors duration-200 ease-out hover:bg-orange-50 dark:hover:bg-white/5"
          >
            ‹
          </button>
          <button
            ...
            className="rounded-full border border-black/10 px-3 py-1.5 text-sm font-medium text-foreground transition-colors duration-200 ease-out hover:bg-orange-50 dark:border-white/10 dark:hover:bg-white/5"
          >
            Today
          </button>
          <button
      ...
      <div className="overflow-hidden rounded-2xl border border-black/5 dark:border-white/10">
        <div className="grid grid-cols-7 border-b border-black/5 bg-[#fff7f1] text-center text-xs font-medium text-zinc-500 dark:border-white/10 dark:bg-[#1e1a15] dark:text-zinc-400">
          ...
          {cells.map((date, i) => {
            if (!date) {
              return (
                <div
                  key={`b${i}`}
                  className="min-h-24 border-b border-r border-black/5 bg-[#fff7f1]/40 dark:border-white/10 dark:bg-white/[0.02]"
                />
              );
            }
            ...
            return (
              <button
                ...
                className={`flex min-h-24 flex-col gap-1 border-b border-r border-black/5 p-1.5 text-left align-top transition-colors duration-200 ease-out hover:bg-orange-50/60 dark:border-white/10 dark:hover:bg-white/5 ${
      ...
      {openDay && openDate && (
        <>
          ...
          <div
            ...
            className="fixed z-50 flex max-h-80 flex-col gap-2 overflow-y-auto rounded-xl border border-black/5 bg-[#fff7f1] p-3 shadow-xl shadow-zinc-900/10 dark:border-white/10 dark:bg-[#1e1a15]"
          >
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                onClick={() => setOpenDay(null)}
                aria-label="Close"
                className="flex h-6 w-6 items-center justify-center rounded-full text-zinc-500 transition-colors duration-200 ease-out hover:bg-orange-50 dark:hover:bg-white/10"
              >
                ✕
              </button>
            </div>
            ...
            {openEvents.length === 0 ? (
              ...
              <ul className="flex flex-col gap-1.5">
                {openEvents.map((e) => (
                  <li
                    key={e.id}
                    className="flex flex-col gap-0.5 rounded-lg border border-black/5 p-2 dark:border-white/10"
                  >
                    <Link
