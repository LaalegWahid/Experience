...
export default async function HostReviewsPage() {
  ...
  return (
    <HostPage
      ...
      action={
        rating.count > 0 ? (
          <div className="flex items-baseline gap-1.5 rounded-full border border-black/5 bg-[#fff7f1] px-4 py-2 dark:border-white/10 dark:bg-[#1e1a15]">
    ...
    >
      {reviews.length === 0 ? (
        <p className="rounded-2xl border border-dashed border-orange-200 bg-[#fff7f1] p-10 text-center text-sm text-zinc-400 dark:border-orange-900/40 dark:bg-[#1e1a15]">
        ...
        <ul className="flex flex-col gap-3">
          {reviews.map((r) => (
            <li
              key={r.id}
              className="flex flex-col gap-2 rounded-2xl border border-black/5 bg-[#fff7f1] p-5 dark:border-white/10 dark:bg-[#1e1a15]"
            >
