...
const inputClass =
  "rounded-lg border border-black/10 bg-transparent px-3 py-2 text-sm text-foreground outline-none transition-colors duration-200 ease-out focus:border-accent dark:border-white/10";
const labelClass =
          ...
          <fieldset className={labelClass}>
            ...
            <div className="flex gap-3">
              {OFFERING_TYPES.map((opt) => (
                <label
                  key={opt.value}
                  className="flex flex-1 cursor-pointer items-center gap-2 rounded-lg border border-black/10 px-3 py-2 text-sm font-normal text-foreground transition-colors duration-200 ease-out hover:bg-orange-50 dark:border-white/10 dark:hover:bg-white/5"
                >
                  <input
                    type="radio"
                    name="type"
  ...
  if (chromeless) {
    ...
    return (
      <form action={formAction} className="flex flex-col gap-6">
        <div className="rounded-2xl border border-black/5 bg-[#fff7f1] p-6 dark:border-white/10 dark:bg-[#1e1a15]">
          {steps.map((s, i) => (
            <div
              key={s.title}
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            <button
              type="submit"
              disabled={pending}
              className="flex h-11 items-center justify-center rounded-full bg-accent px-6 text-sm font-medium text-accent-foreground transition-all duration-200 ease-out hover:-translate-y-0.5 hover:opacity-90 disabled:pointer-events-none disabled:opacity-60"
            >
    ...
    return (
      <form action={formAction} className="flex flex-col gap-6">
        {steps.map((s) => (
          <section
            key={s.title}
            className="flex flex-col gap-5 rounded-2xl border border-black/5 bg-[#fff7f1] p-6 dark:border-white/10 dark:bg-[#1e1a15]"
          >
    ...
    <form ref={formRef} action={formAction} className="flex flex-col gap-6">
              ...
              <span
                className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-semibold transition-colors duration-200 ease-out ${
                  i <= step
                    ...
                    : "bg-[#fff7f1] text-zinc-400 dark:bg-white/10"
                }`}
              >
                {i + 1}
              </span>
      ...
      <div className="flex items-center justify-between">
        <button
          ...
          onClick={goBack}
          disabled={step === 0}
          className="rounded-full border border-black/10 px-5 py-2 text-sm font-medium text-foreground transition-all duration-200 ease-out hover:-translate-y-0.5 hover:bg-orange-50 disabled:pointer-events-none disabled:opacity-40 dark:border-white/10 dark:hover:bg-white/5"
        >
          Back
        </button>
        {isLast ? (
