  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-8">
      ...
      <div className="flex flex-col gap-3">
        <Link
          href="/host"
          className="w-fit text-sm text-zinc-500 transition-colors hover:text-foreground dark:text-zinc-400"
        >
          ← Back to dashboard
        </Link>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <aside className="flex flex-col gap-6 lg:sticky lg:top-24 lg:self-start">
          ...
          <Link
            ...
            className="flex items-center justify-between gap-4 rounded-2xl border border-zinc-200 bg-background p-5 transition-colors hover:border-accent/40 dark:border-zinc-800 dark:hover:border-accent/40"
          >
            ...
            <div className="flex flex-col gap-2">
              {offering.status === "published" ? (
                <form action={unpublishOffering.bind(null, offering.id)}>
                  <button
                    type="submit"
                    className="w-full rounded-full border border-zinc-300 px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-900"
                  >
                    Unpublish
                  </button>
                </form>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <form action={publishOffering.bind(null, offering.id)}>
                    <button
                      type="submit"
                      className="w-full rounded-full bg-foreground px-4 py-2 text-sm font-medium text-background transition-colors hover:bg-zinc-700 dark:hover:bg-zinc-200"
                    >
                      Request to publish
                    </button>
                  </form>
              ...
              {offering.status === "published" && (
                <Link
                  ...
                  className="flex w-full items-center justify-center rounded-full border border-zinc-300 px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-900"
                >
                  View public page
                </Link>
              )}
          ...
          {/* Danger zone */}
          <div className="flex flex-col gap-3 rounded-2xl border border-red-200 bg-background p-5 dark:border-red-900/50">
