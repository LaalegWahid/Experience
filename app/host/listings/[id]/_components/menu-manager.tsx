...
export function MenuManager({
  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-black/5 bg-[#fff7f1] p-6 dark:border-white/10 dark:bg-[#1e1a15]">
          ...
          {items.map((item) => (
            <li key={item.id} className="flex items-center gap-3 py-3">
              ...
              <form action={removeAction}>
                ...
                <button
                  type="submit"
                  className="rounded-full border border-black/10 px-3 py-1.5 text-xs font-medium text-foreground transition-colors duration-200 ease-out hover:bg-orange-50 dark:border-white/10 dark:hover:bg-white/5"
                >
                  Remove
                </button>
              </form>
      ...
      >
        ...
        <button
          type="submit"
          disabled={pending}
          className="w-fit rounded-full bg-accent px-5 py-2 text-sm font-medium text-accent-foreground transition-all duration-200 ease-out hover:-translate-y-0.5 hover:opacity-90 disabled:pointer-events-none disabled:opacity-60"
        >
